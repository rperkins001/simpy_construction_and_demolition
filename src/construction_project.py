...
class ConstructionProject(BaseProject):
    ...
    def setup(self):
        super().setup()
        # Set up resources specific to the construction project
        ...
        equipment_list = generate_equipment(self.env, self.equipment_specs, self.resources["fuel_station"])
        self.material_processor = MaterialProcessor(self.env, equipment_list, self.processing_time_range, self.storage_capacity)
    ...
    def run(self):
        ...
        print(f"Starting construction project: {self.name}")
    ...
    def daily_tasks(self, day):
        ...
        workers_request = self.resource_allocator.allocate_resources("workers", 10)
        trucks_request = self.resource_allocator.allocate_resources("trucks", 5)
